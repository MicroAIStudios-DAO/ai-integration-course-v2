...
// Layout and page components are code-split via React.lazy so the initial
// bundle only ships the landing route. Each route now resolves its own chunk
// on demand, keeping first-load JS small. Always-mounted chrome (ConsentBanner,
// UserJotWidget, ExitIntentModal) stays eager since it renders on every route.
...
// Lightweight fallback shown while a route chunk loads. Kept minimal and
// dependency-free so it adds no weight to the initial bundle.
